...
const operations: any = {
    0: (str: string, direction: "F" | "B", seed: number) => {
        debug && console.log(`CALL 0 ${direction} ${seed}`)
        return str.split("").reverse().join("")
    },
    1: (str: string, direction: "F" | "B", seed: number) => {
    ...
    2: (str: string, direction: "F" | "B", seed: number) => {
        debug && console.log(`CALL 2 ${direction} ${seed}`)
        return swapChar(str, 0, seed % str.length)
    },
}

const chunkify: (str: string, size: number) => string[] = (str, size) => {
    const chunks = []
...
const swapChar: (str: string, fromIndex: number, toIndex: number) => string = (str, fromIndex, toIndex) => {
    const arr = str.split("")
...
const stringToLongNumber: (inputString: string) => string = (inputString) => {
    let longNumber = '';
    [...inputString].map(i => {
...
const longNumberToString: (longNumber: string) => string = (longNumber) => {
    let stringResult = ''
...
const modifyCharBySeed: (char: string, seed: string, modifier?: number) => string | null = (char, seed, modifier = 1) => {
...
const compactZeroes: (number: string) => string = (number) => {
...
const decompactZeroes: (number: string) => string = (number) => {
...
const compressPixel: (pixel: string) => string = (pixel) => {
...
const decrompressPixel: (pixel: string) => string = (pixel) => {
